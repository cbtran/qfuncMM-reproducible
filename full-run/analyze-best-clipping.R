...
n_timept <- 60
setting <- paste0("high-mid-M", n_timept, "-100")

result <- readRDS(paste0("full-run/", setting, "-result.rds"))$stage2
clipped <- readRDS(paste0("full-run/", setting, "-result-clipped.rds"))$stage2
best <- readRDS(paste0("full-run/", setting, "-result-best.rds"))$stage2
bestclipped <- readRDS(paste0("full-run/", setting, "-result-best-clipped.rds"))$stage2

nsim <- 100

# Reshape data to long format
df_result <- tidyr::gather(data.frame(t(result[1, , ])),
                           key = "pair", value = "value")
df_result$method <- "ReML"
df_clipped <- tidyr::gather(data.frame(t(clipped[1, , ])),
                            key = "pair", value = "value")
df_clipped$method <- "Clipped"
df_best <- tidyr::gather(data.frame(t(best[1, , ])),
                         key = "pair", value = "value")
df_best$method <- "Best"
df_bestclipped <- tidyr::gather(data.frame(t(bestclipped[1, , ])),
                                key = "pair", value = "value")
df_bestclipped$method <- "Best clipped"
...
ggdf <- rbind(dfca, df_result, df_clipped, df_best, df_bestclipped)
...
ggsave(paste0("full-run/", setting, "-best-clip-plot.png"))
