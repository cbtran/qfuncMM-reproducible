...
# Usage: ./run_stage2_vecchia.sh <data_spec> <cov_setting> <data_dir> <out_dir> <delta> <psi>
...
if [ -z "$data_spec" ] || [ -z "$cov_setting" ] || [ -z "$data_dir" ] || [ -z "$out_dir" ] || [ -z "$delta" ] || [ -z "$psi" ]; then
  echo "Usage: $0 <data_spec> <cov_setting> <data_dir> <out_dir> <delta> <psi>"
  exit 1
fi

Rscript R_files/simulation/run_stage2.R "$data_spec" "$cov_setting" "$data_dir" "$out_dir" TRUE 100 "$delta" "$psi"