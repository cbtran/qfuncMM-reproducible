...
opt_simulation_3region <- function(
  ...
  X_Region1 <- data$X_Region1
  X_Region2 <- data$X_Region2
  X_Region3 <- data$X_Region3

  # Correlation of Averages from Chao's code
  # Vanilla
  X_R1_mat <- matrix(X_Region1, nrow = n_timept)
  X_R2_mat <- matrix(X_Region2, nrow = n_timept)
  X_R3_mat <- matrix(X_Region3, nrow = n_timept)
  ...
  # Bspline
  X_R1_df <- data.frame(signal=X_Region1, time=rep(1:n_timept, times=L1))
  X_R2_df <- data.frame(signal=X_Region2, time=rep(1:n_timept, times=L2))
  X_R3_df <- data.frame(signal=X_Region3, time=rep(1:n_timept, times=L3))

  fitR1 <- lm(signal ~  -1 + bs(time, df=n_bspline, intercept=T), data=X_R1_df)
  fitR2 <- lm(signal ~  -1 + bs(time, df=n_bspline, intercept=T), data=X_R2_df)
  fitR3 <- lm(signal ~  -1 + bs(time, df=n_bspline, intercept=T), data=X_R3_df)
  ...
  dist_sqrd_mat_region1 <- get_dist_sqrd_mat(n_voxel, side_length, voxids[, 1])
  ...
  result_region1 <- opt_intra(
    parametersInit_region1, X_Region1, G_region,
    dist_sqrd_mat_region1, time_sqrd_mat, L1, n_timept, kernel_type)
