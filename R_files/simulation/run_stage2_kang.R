...
if (length(args) < 5) {
  stop("Usage: Rscript run_stage2_kang.R <data_spec> <cov_setting> <data_dir> <out_dir> <seed>")
}
...
out_dir_spec <- file.path(out_dir, data_spec, "stage2_kang")
if (cov_setting == "noiseless") {
...
for (s in seq_len(nrow(all_settings))) {
  ...
  for (simid in seq_along(data_setting$data)) {
    ...
    for (rp in reg_pairs) {
      ...
      gamma_hat_1 <- compute_eblup_gamma(r1_data_std, r1_coords, j1$eblue, j1$stage1)
      gamma_hat_2 <- compute_eblup_gamma(r2_data_std, r2_coords, j2$eblue, j2$stage1)
