...
read_results <- function(result_folder) {
  pair.pearson <- read_csv(paste0(result_folder, "/rho_pearson.csv"))
  pair.inter <- read_csv(paste0(result_folder, "/rho_inter.csv"))
...
med_weak <- cbind(create_boxplot(med_signal.weak_intra, "med_signal_weak_intra"), signal = "med", intra="weak")

# med signal - strong intra
med_signal.strong_intra <- read_results(paste0(output_dir, "med-signal-strong-intra"))
med_strong <- cbind(create_boxplot(med_signal.strong_intra, "med_signal_strong_intra"), signal = "med", intra="strong")

################### Strong signal ###############3

# strong signal - weak intra
strong_signal.weak_intra <- read_results(paste0(output_dir, "strong-signal-weak-intra"))
strong_weak <- cbind(create_boxplot(strong_signal.weak_intra, "strong_signal_weak_intra"), signal = "strong", intra="weak")

# strong signal - strong intra
strong_signal.strong_intra <- read_results(paste0(output_dir, "strong-signal-strong-intra"))
strong_strong <- cbind(create_boxplot(strong_signal.strong_intra, "strong_signal_strong_intra"), signal = "strong", intra="strong")



boxplot_df <- rbind(med_weak, med_strong, weak_weak, weak_strong, strong_weak, strong_strong)
boxplot_df$signal <- factor(boxplot_df$signal, levels=c("weak",  "med", "strong"))
...
levels(boxplot_df$signal) <- c(weak = expression(k[eta] == 0.5),
                               med = expression(k[eta] == 1),
                               strong = expression(k[eta] == 1.5))
levels(boxplot_df$intra) <- c(weak = expression(phi[gamma] == 1),
...
fig1 <- ggplot(boxplot_pair12,
  aes(y = rho, x = fct_rev(type), fill = fct_rev(type))) +
  geom_boxplot() +
  geom_hline(
    yintercept = true_rho[1], linewidth=1, color="red", linetype='dashed') +
  ylab(expression(rho)) +
