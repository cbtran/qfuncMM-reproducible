...
for (delta in c("high", "mid", "low")) {
  for (psi in c("high", "mid", "low")) {
    ...
    outfile <- file.path(s2_dir, sprintf("asymp_ci_%s-%s_diag.csv", delta, psi))
    ...
    for (id in seq_len(nrow(stage2_out))) {
      ...
      avrho <- qfuncMM::get_asymp_var_rho(theta, r1_stage1_out, r2_stage1_out, method = "vecchia_diag_time")
