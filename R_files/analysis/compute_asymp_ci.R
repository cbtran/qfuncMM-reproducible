...
# args <- c("out", "std", "stage2_reml", "reml")
outdir <- args[1]
dataspec <- args[2]
s2_run_str <- args[3]
method <- args[4]
...
stopifnot(delta %in% c("high", "mid", "low"))
stopifnot(psi %in% c("high", "mid", "low"))
...
for (id in seq_len(nrow(stage2_out))) {
  ...
  avrho <- stage2_out$asympvar_rho.rho[[id]]
  if (method == "vecchia") {
    avrho <- qfuncMM::get_asymp_var_rho(theta, r1_stage1_out, r2_stage1_out, method = method)
  }
